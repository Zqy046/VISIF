...
class Exp_CVV_TSContext(Exp_Basic):
    def __init__(self, args):
        super(Exp_CVV_TSContext, self).__init__(args)
        
    def _build_model(self):
        model = self.model_dict[self.args.model].Model(self.args)
        self.tokenizer = InternLM2Tokenizer.from_pretrained(self.args.llm_ckp_dir, use_fast=False)
        special_tokens_dict = {'additional_special_tokens': ['<TIME_SERIES>']}
        self.tokenizer.add_special_tokens(special_tokens_dict)
        model.internvl.language_model.resize_token_embeddings(len(self.tokenizer), mean_resizing=False)
        
        img_context_token_id = self.tokenizer.convert_tokens_to_ids('<IMG_CONTEXT>')
        model.internvl.img_context_token_id = torch.tensor(img_context_token_id, dtype=torch.long)
        ts_context_token_id = self.tokenizer.convert_tokens_to_ids('<TIME_SERIES>')
        model.internvl.ts_context_token_id = torch.tensor(ts_context_token_id, dtype=torch.long)
        self.num_image_token = model.internvl.num_image_token
    ...
    def _get_data(self, flag):
        ...
        For CVV dataset with context (images), uses TSContextDataset.
        """
        if flag == 'test' and self.args.data == 'cvv_tscontext':
            # Support multiple test sets for CVV dataset
            test_sets = []
            test_loaders = []
            for test_flag in ['test1', 'test2', 'test3']:
                data_set, data_loader = data_provider(self.args, test_flag, tokenizer=self.tokenizer, num_image_token=self.num_image_token)
                test_sets.append(data_set)
                test_loaders.append(data_loader)
            return test_sets, test_loaders
        else:
            data_set, data_loader = data_provider(self.args, flag, tokenizer=self.tokenizer, num_image_token=self.num_image_token)
    ...
    def prepare_batch(self, batch):
        """
        Prepare batch data from TSContextDataset dictionary format to model input format.
        Extract GHI channel from timeseries and get pixel_values (preprocessed images).
        """
        timeseries = batch["timeseries"]  # [bs, seq_len, n_channels]
        ghi_idx = batch["ghi"][0].long()[0].item()
        x_ts = timeseries[:, :, ghi_idx:ghi_idx+1].float()  # [bs, seq_len, 1]
        y_ts = batch["target"].float()  # [bs, label_len + pred_len, 1]

        pixel_values = batch["context"].float()  # [bs, seq_len, num_tiles, C, H, W]

        if "x_timestamp" in batch:
            x_timestamp = batch["x_timestamp"].float()  # [bs, token_num, hidden_dim]
        else:
            x_timestamp = None
        # y_timestamp暂时不需要
        y_timestamp = None
        ...
        return x_ts, y_ts, x_timestamp, y_timestamp, pixel_values, ghi_idx, input_ids
    ...
    def vali(self, vali_data, vali_loader, criterion, is_test=False):
        ...
        with torch.no_grad():
            for i, batch in enumerate(vali_loader):
                iter_count += 1
                batch_x, batch_y, batch_x_mark, batch_y_mark, batch_pixel_values, ghi_id, input_ids = self.prepare_batch(batch)
                ...
                # 如果有timestamp embedding，也需要移到device
                ...
                if self.args.use_amp:
                    with torch.cuda.amp.autocast():
                        outputs = self.model(batch_x, batch_x_mark, None, batch_y_mark, batch_pixel_values, input_ids)
                else:
                    outputs = self.model(batch_x, batch_x_mark, None, batch_y_mark, batch_pixel_values, input_ids)
    ...
    def test(self, setting, test=0):
        ...
        if self.args.data == 'cvv_tscontext' and isinstance(test_loader, list):
        ...
        # Save JSON results
        if (self.args.use_multi_gpu and self.args.local_rank == 0) or not self.args.use_multi_gpu:
            if self.args.data == 'cvv_tscontext':
                file_path = f"./test_results/{setting}"
                os.makedirs(file_path, exist_ok=True)

                import re
                match = re.search(r'InternVL2-[^_]+', setting)
                internvl_str = match.group(0) if match else None
                ...
                for name in station_names:
                    if name in setting.split('_'):
                        ...
                        break
                if internvl_str is not None:
                    filename = os.path.join(file_path, f"test_{internvl_str}{training_station_name}.json")
                else:
    ...
    def _test_single(self, test_loader, setting, test_name='test', station_name='test'):
        ...
        with torch.no_grad():
            for i, batch in enumerate(test_loader):
                ...
                if (self.args.use_multi_gpu and self.args.local_rank == 0) or not self.args.use_multi_gpu:
                    if self.args.data == 'cvv_tscontext' and station_name in ['IZA', 'CNR', 'PAL']:
